...
export default function ShowListChat({
    ...
    const getMostWords = useCallback(() => {
        return data.filter(d => d.data.comment.toLowerCase().includes(word.toLowerCase())).map(({ data }) => ({
            comment: data.comment, time: data.createTime, user: {
    ...
    const columns: ColumnDef<OutputType>[] = useMemo(() => [
        ...
        },
        {
            ...
            cell: ({ getValue }: CellContext<OutputType, unknown>) => {
                const val = getValue() as string
                return moment(moment.unix(Math.round(parseInt(val) / 1000))).format("hh:mm:ss")
            },
        },
        {
            accessorKey: "user",
            ...
            cell: ({ getValue }: CellContext<OutputType, unknown>) => {
                ...
                return <Link href={`https://tiktok.com/@${val.uniqueId}`} target="_blank" className="underline">{showUsername ? `@${val.uniqueId}` : val.nickname}</Link>
            },
        }
    ], [showUsername, data]);

    const mostWordsArray = useMemo(() => selectedUsername == "" ? getMostWords() : getMostWords().filter(({ user }) => user.uniqueId == selectedUsername), [data, selectedUsername]);
    ...
    return (
        <Dialog open={dialogOpen} onOpenChange={(e) => {
            ...
            {!setDialogOpenState && (<DialogTrigger asChild>
                {TriggerElement ? TriggerElement : <Button size={'icon'} variant={'outline'}><ArrowRightFromLineIcon /></Button>}
            </DialogTrigger>)}
            <DialogContent className="sm:max-w-[625px] min-h-[500px] flex flex-col gap-3">
                <DialogHeader>
                    <DialogTitle>List Comment</DialogTitle>
                    <DialogDescription>
                        Showing comment {word && <>containing <span className="font-bold">{word}</span></>} {selectedUsername && <>from @{selectedUsername}</>}
                    </DialogDescription>
                </DialogHeader>
                {(username == "") && (
                    <div className="flex items-center gap-2">
                        <Switch checked={showUsername} onCheckedChange={(e) => { setShowUsername(e); }} />
                        <Label>Show username</Label>
                    </div>)}
                {(username == "") && <Popover open={open} onOpenChange={setOpen}>
                    <PopoverTrigger asChild>
                        ...
                        >
                            {selectedUsername
                                ? listUserArray.find((user) => user.uniqueId === selectedUsername)?.uniqueId
                                : "Select user..."}
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <Check
                                                    className={cn(
                                                        "mr-2 h-4 w-4",
                                                        selectedUsername === user.uniqueId ? "opacity-100" : "opacity-0"
                                                    )}
                                                />
                                                {user.nickname}</div> <span className="text-sm text-muted-foreground">@{user.uniqueId}</span>
                                        </CommandItem>
                                    ))}
                                </CommandGroup>
                            </CommandList>
